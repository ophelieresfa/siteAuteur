...
<div class="ffp_customer_details">
    ...
    <div  class="ffp_submission_details">
        <ul>
            ...
            <li><?php _e('Customer Name:', 'fluentformpro');?> <?php echo esc_html($transaction->payer_name); ?></li>
            <?php endif; ?>
            <?php if($transaction->payer_email): ?>
            <li><?php _e('Customer Email:', 'fluentformpro');?> <?php echo esc_html($transaction->payer_email); ?></li>
            <?php endif; ?>
            <?php if($transaction->billing_address && strlen($transaction->billing_address) > 2): ?>
            <li><?php _e('Billing Address:', 'fluentformpro');?> <?php echo esc_html($transaction->billing_address); ?></li>
            <?php endif; ?>
            <?php if($transaction->shipping_address && strlen($transaction->shipping_address) > 2): ?>
            <li><?php _e('Shipping Address:', 'fluentformpro');?> <?php echo esc_html($transaction->shipping_address); ?></li>
            <?php endif; ?>
        </ul>
    </div>
</div>
