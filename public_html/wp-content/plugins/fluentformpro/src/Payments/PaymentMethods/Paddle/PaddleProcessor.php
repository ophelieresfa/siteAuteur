...
class PaddleProcessor extends BaseProcessor
{
    ...
    public function confirmModalPayment()
    {
        ...
        if ($checkoutId) {
            ...
            // Verify amount matches expected transaction total
            $paddleTotal = intval(ArrayHelper::get($verifiedTransaction, 'data.details.totals.total', 0));
            if ($transaction->payment_total && $paddleTotal && $paddleTotal !== intval($transaction->payment_total)) {
                ...
                if ($strict) {
                    ...
                    wp_send_json([
                        'errors' => __('Payment amount mismatch detected. The payment has been marked for review.', 'fluentformpro')
                    ], 423);
                }
            }
