...
?>
<a href="#!" class="custom-pagination__btn custom-pagination__btn ir-prev-page <?php esc_attr_e($prevClass); ?> <?php echo "ir-" . esc_attr($type) . "-pagination"; ?>">
...
for($i=0; $i<$countPages; $i++) {
    if($i==0 || $i==1 || $i==$countPages-2 || $i==$countPages-1 || $i==$currentOffset-1 || $i==$currentOffset || $i==$currentOffset+1) {
        $pagesToShow[] = $i;
    } else {
        if(end($pagesToShow) !== 'ellipsis') {
            $pagesToShow[] = 'ellipsis';
        }
    }
}

foreach ($pagesToShow as $i) {
    if (is_string($i) && $i == 'ellipsis') {
        ?>
        <span class="custom-pagination__ellipsis">...</span>
        <?php
    } else {
        $page = $i + 1;
